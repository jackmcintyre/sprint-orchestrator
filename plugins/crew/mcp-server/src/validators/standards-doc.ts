 ...
 * with `maximum === 10`.
 */
function isCriteriaCapViolation(issue: z.ZodIssue): boolean {
  if (issue.code !== "too_big") return false;
  if (issue.path.length === 0 || issue.path[0] !== "criteria") return false;
...
export function parseStandardsDoc(raw: string, sourcePath: string): StandardsDoc {
  ...
  if (!parsed.success) {
    ...
    if (isCriteriaCapViolation(first)) {
      const actual = Array.isArray((parsedYaml as { criteria?: unknown[] })?.criteria)
        ? (parsedYaml as { criteria: unknown[] }).criteria.length
        : "unknown";
